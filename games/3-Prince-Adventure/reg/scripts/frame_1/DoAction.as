...
function popup(pmErr, pmRestore)
{
   ...
   restore = pmRestore;
   var _loc3_ = 1;
   while(_loc3_ <= restore.length)
   {
      restore[_loc3_ - 1].gotoAndStop(1);
      restore[_loc3_ - 1].mc_over.gotoAndStop(1);
      _loc3_ += 1;
   }
...
function correctEmailFormat(em)
{
   ...
   if(_loc2_ > 0 && _loc3_ > 0 && _loc2_ < em.length - 1 && _loc2_ - _loc3_ > 1 && em.lastIndexOf("@",_loc3_ - 1) < 0 && em.split(" ").length == 1 && em.substr(_loc3_ + 1,1) != ".")
   {
      return true;
   }
   return false;
